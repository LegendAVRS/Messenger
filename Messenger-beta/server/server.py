...
group_exist = "?[SERVER] Group name has already existed"
group_not_exist = "?[SERVER] Group name does not exist"
is_member = "?[SERVER] This member is in your group"
group_ok = "?[SERVER] You have successfully created a group: "
not_member = "?[SERVER] You are not a member in this group"
...
def Avatar(client, name):
    ...
    with open(path, "rb") as image:
        ...
        while image_data:
            ...
            image_data = image.read(512)
    time.sleep(1)
    send_messages(client, f"ENDofFILE{name}")
    time.sleep(0.5)
...
def New_Group(client, name, username):
    ...
    first_msg = f"[SERVER] {username} has created the group\n"
    ...
    Which_group_are_you_in(client, username)
...
def group_chat(client, username, name):
    ...
    while True:
        ...
        if len(temp) == 2 and temp[0][0] == "/":
            ...
            msg = f"?[SERVER] {name} has added {invited_member} to the group\n"
            changed = True
...
def Online_list_display(client, name):
    ...
    send_messages(client, f"!CONNECT {username_list}")
    time.sleep(1)
...
def List_display(client, name):
    ...
    for username in client_info:
        if username != "You" and username != "SERVER":
            username_list += username + ' '
    username_list = username_list[:-1]
    send_messages(client, f"!ADD {username_list}")
    time.sleep(1)
...
def Which_group_are_you_in(client, name):
    ...
    if len(group_list) == 0:
        return
    send_messages(client, f"!GROUP {group_list}")
    time.sleep(1)
...
def client_and_server(client, addr):
    ...
    time.sleep(1)
    # Gửi avatar của bản thân
    Avatar(client, user.username)

    # Gửi avatar những người khác
    for username in client_info:
        if username == "You":
            continue
        if username == "SERVER":
